...
const Online = ({ navigation }) => {
  const [currentVideoIndex_online, setCurrentVideoIndex_online] = useState(0);
  const videoListRef_online = useRef(null);
  const [paused_online, setPaused_online] = useState(true);
  ...
  useEffect(() => {
    return () => {
      setPaused_online(true);
    };
  }, []);

  useFocusEffect(
    React.useCallback(() => {
      setPaused_online(false);
      return () => {
        setPaused_online(true);
      };
    }, [])
  ...
  const togglePause_onlinemend = () => {
    setPaused_online(!paused_online);
  };
  
  const onPageSelected = (event) => {
    setCurrentVideoIndex_online(event.nativeEvent.position);
  };
  ...
  return (
    ...
    >
      {videos_online.map((el, index) => (
        <Pressable key={el.id} style={{ backgroundColor: 'black' }} onPress={() => togglePause_onlinemend()}>
          <Video
            ...
            repeat={true}
            resizeMode="contain"
            paused={index !== currentVideoIndex_online || paused_online}
            ref={videoListRef_online}
            onLoadStart={() => {
              setCurrentVideoIndex_online(index);
            }}
          />
