...
const UserFollow = ({ navigation }) => {
  const [currentVideoIndex_follow, setCurrentVideoIndex_follow] = useState(0);
  const videoListRef_follow = useRef(null);
  const [paused_follow, setPaused_follow] = useState(true);
  ...
  useEffect(() => {
    return () => {
      setPaused_follow(true);
    };
  }, []);

  useFocusEffect(
    React.useCallback(() => {
      setPaused_follow(false);
      return () => {
        setPaused_follow(true);
      };
    }, [])
  ...
  const togglePause_follow = () => {
    setPaused_follow(!paused_follow);
  };
  const onPageSelected = (event) => {
    setCurrentVideoIndex_follow(event.nativeEvent.position);
  };
  ...
  return (
    ...
    >
      {videos_follow.map((el, index) => (
        <Pressable key={el.id} style={{ backgroundColor: 'black' }} onPress={() => togglePause_follow()}>
          <Video
            ...
            repeat={true}
            resizeMode="contain"
            paused={index !== currentVideoIndex_follow || paused_follow}
            ref={videoListRef_follow}
            onLoadStart={() => {
              setCurrentVideoIndex_follow(index);
            }}
          />
